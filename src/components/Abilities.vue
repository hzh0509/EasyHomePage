                    ...
                    <div v-for="j in 2" :key="j" class="col-md-6 col-12 mb-5">
                        <div class="card jn-card jn-abilities-card h-100 jn-animate-card">
                            <div class="card-body" v-if="markdownFiles[(i-1) * 2 + (j-1)]">
                                <div class="jn-icon"><i class="bi" :class="markdownFiles[(i-1) * 2 + (j-1)].attributes.icon"></i></div>
...
import { attributes, html } from '@content/abilities/index.md';
...
const markdownContext = import.meta.glob('@content/abilities/!(index).md');
...
<style scoped>
.jn-abilities { background-color: rgba(58, 232, 179, 0.08); }
.jn-h2 { font-size: 3rem; margin-bottom: 2rem; }
.jn-fit { width: fit-content; padding: 0.5rem 1rem; border: 2px solid var(--bs-dark); }
</style>